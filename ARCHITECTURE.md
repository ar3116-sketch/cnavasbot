...
SQLite stores academic state, evidence, jobs, usage, and encrypted-credential references. Electron `safeStorage` protects provider keys at rest using the OS facility available to Electron. Passwords, browser cookies, and raw keys are never returned by REST or MCP. Canvas cookies remain in Electron's dedicated persistent profile. In demo mode, the persisted job runner exercises the same scan schema and reconciliation pipeline without a network or paid model call; live jobs are left for the desktop computer-use worker to claim.
